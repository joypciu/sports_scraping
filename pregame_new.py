...
class EnhancedIntelligentScraper:
    def __init__(self, headless: bool = True, load_wait: int = 2000, max_scrolls: int = 15, scroll_pause: int = 300):
        ...
        self.sport_patterns = {
            ...
            "Tennis": {
             ...
             "PGA": {
                "tabs": ["PGA TOUR", "PGA Tour", "Golf", "PGA"],
                "fixture_classes": ["cpm-ParticipantLabelGolfScore"],
                "team_class": "_Name",
                "time_class": "",  # Golf doesn't have traditional match times
                "expected_odds": ["to_win"],
                "logo_indicators": ["pga", "golf"],
                "validation_keywords": ["tiger", "spieth", "mcilroy", "championship", "tournament"],
        ...
        self.debug_dir = self.outputs_dir / "debug"
        
        # Create output directories
        for dir_path in [self.outputs_dir, self.html_dir, self.debug_dir]:
    ...
    async def find_sport_grid(self, page, sport: str):
        ...
        try:
            ...
            if best_grid:
                ...
                grid_html = await best_grid.inner_html()
                debug_file = self.html_dir / f"{sport}_{self.session_id}.html"
                debug_file.write_text(grid_html, encoding="utf-8")
    ...
    async def extract_sport_games(self, page, sport: str) -> List[Game]:
        ...
        if sport == "MLB":
            return await self.extract_mlb_games(grid, sport)
        elif sport == "NBA":
            return await self.extract_nba_games_fixed(page, sport)
        elif sport == "NBA2K":
            return await self.extract_nba2k_games(page, sport)
        elif sport == "NHL":
            return await self.extract_nhl_games(grid, sport)
        elif sport == "NCAAF":
            return await self.extract_ncaaf_games(page, sport)  # NCAAF uses page-based approach for spotlight sections
        elif sport == "NFL":
            return await self.extract_nfl_games(page, sport)  # NFL uses page-based approach
        elif sport == "Tennis":
            return await self.extract_tennis_games(grid, sport)
        elif sport == "CFL":
            return await self.extract_cfl_games(page, sport)  # CFL uses page-based approach
        elif sport == "PGA":
            return await self.extract_pga_games(page, sport)  # PGA uses page-based approach
        elif sport == "UFC":
            return await self.extract_ufc_games(page, sport)  # UFC uses page-based approach
        elif sport == "Soccer":
            return await self.extract_soccer_games(page, sport)  # Soccer uses page-based approach
        elif sport == "EPL":
            return await self.extract_epl_games(page, sport)  # EPL uses page-based approach
        elif sport == "MLS":
            return await self.extract_mls_games(page, sport)  # MLS uses page-based approach  
        else:
            return await self.extract_generic_games(grid, sport)
        ...
        try:
            ...
            extraction_result = await grid.evaluate("""
                (grid) => {
                    ...
                    // Step 1: Find Tennis section by looking for tennis icon/logo
                    let tennisSection = null;
                    
                    // Method 1: Look for tennis logo in style background-image
                    const headerImages = document.querySelectorAll('.cpm-Header_HeaderImage, .ss-HomeSpotlightHeader_HeaderImage');
                    for (const img of headerImages) {
                        const style = img.getAttribute('style') || '';
                        if (style.includes('zClass_Tennis.svg') || style.includes('tennis')) {
                            tennisSection = img.closest('.cpm-CouponPodModule, .ss-HomepageSpotlight');
                            break;
                        }
                    }
                    ...
                    console.log('Found Tennis section by icon detection');
    ...
    async def extract_pga_games(self, grid, sport: str) -> List[Game]:
        """Fast PGA Tour extraction using icon-based detection"""
        games = []

        try:
            self.logger.info("🏌️ Starting fast PGA Tour extraction...")

            # Fast PGA extraction using tournament icon detection
            extraction_result = await grid.evaluate("""
                (grid) => {
                    const results = [];

                    // Step 1: Find PGA section by looking for PGA logo icon
                    let pgaSection = null;

                    // Method 1: Look for PGA logo in style background-image
                    const headerImages = document.querySelectorAll('.cpm-Header_HeaderImage, .ss-HomeSpotlightHeader_HeaderImage');
                    for (const img of headerImages) {
                        const style = img.getAttribute('style') || '';
                    ...
                    // Method 2: Look for PGA/Golf text as backup
                    if (!pgaSection) {
                        const headers = document.querySelectorAll('.cpm-Header_Title, .ss-HomeSpotlightHeader_Title');
                        for (const header of headers) {
                    ...
                    if (!pgaSection) {
                        console.log('PGA section not found by icon/text detection');
                        return [];
                    }

                    console.log('Found PGA section by icon detection');
                    ...
                    // Step 3: Extract players from PGA section
                    const playerElements = Array.from(pgaSection.querySelectorAll('.cpm-ParticipantLabelGolfScore_Name'));

                    console.log(`Found ${playerElements.length} PGA players`);

                    // Step 4: Extract "To Win" odds
                    const toWinOdds = [];
                    const oddsContainer = pgaSection.querySelector('.cpm-HScrollPlaceColumnMarketGolf');
                    if (oddsContainer) {
                        const oddsElements = oddsContainer.querySelectorAll('.cpm-ParticipantOdds_Odds');
                        for (const oddsElem of oddsElements) {
                            const oddsText = oddsElem.textContent;
                            if (oddsText) {
                                toWinOdds.push(oddsText.trim());
                            }
                        }
                    }

                    console.log(`Found ${toWinOdds.length} To Win odds`);

                    // Step 5: Match players with odds (1:1 mapping)
                    for (let i = 0; i < playerElements.length; i++) {
                        ...
                        const playerOdds = i < toWinOdds.length ? toWinOdds[i] : null;

                        // Create player entry (PGA is individual, not team vs team)
                        results.push({
                            player: playerName,
                            tournament: tournamentName,
                            date: new Date().toISOString().split('T')[0],
                            time: 'Tournament',
                            to_win_odds: playerOdds ? [playerOdds] : [],
                            fixture_id: `pga_${i}`
                        });
                    }
            ...
            # Process results into Game objects (adapting PGA structure to Game format)
            for result in extraction_result:
                # For PGA, we treat each player as "team1" vs "Field" as "team2"
                odds_obj = GameOdds(
                    spread=[],
                    total=[],
                    moneyline=result['to_win_odds']  # Use to_win_odds as moneyline
                )

                game = Game(
                    sport="PGA",
                    team1=self.clean_text(result['player']) or f"Player_{len(games)}",  # Player name
                    team2=f"Field ({result['tournament']})",   # Tournament field
                    date=self.clean_text(result['date']),
                    time=self.clean_text(result['time']),
                    odds=odds_obj,
                    fixture_id=result['fixture_id']
        ...
        self.logger.info(f"PGA: Extracted {len(games)} players with fast method")
        return games
    ...
    async def extract_soccer_games(self, page, sport: str) -> List[Game]:
        """Soccer extraction using improved fixture detection with eSoccer filtering"""
        games = []

        try:
            self.logger.info("⚽ Starting Soccer extraction...")

            # Soccer extraction using direct page extraction after navigation
            extraction_result = await page.evaluate("""
                () => {
                    const results = [];
                    const seenMatches = new Set(); // Track unique matches within soccer extraction

                    console.log('⚽ Starting Soccer extraction...');

                    // DEBUG: Log all sections found on the page
                    const allSections = document.querySelectorAll('.cpm-CouponPodModule, .ss-HomepageSpotlight');
                    console.log(`📊 Total sections found: ${allSections.length}`);

                    for (let i = 0; i < allSections.length; i++) {
                        const section = allSections[i];
                        const title = section.querySelector('.cpm-Header_Title, .ss-HomeSpotlightHeader_Title');
                        const titleText = title ? title.textContent.trim() : 'No title';
                        const headerImg = section.querySelector('.cpm-Header_HeaderImage, .ss-HomeSpotlightHeader_HeaderImage');
                        const imgStyle = headerImg ? headerImg.getAttribute('style') || '' : '';
                        console.log(`Section ${i}: "${titleText}" - Icon: ${imgStyle.includes('zClass_Soccer.svg') ? 'SOCCER' : imgStyle.includes('zClass_ESports.svg') ? 'ESPORTS' : 'OTHER'}`);
                    }

                    // Step 1: Find ALL Soccer fixtures on the page (more comprehensive approach)
                    let allFixtures = [];

                    // Method 1: Direct soccer fixture elements (most reliable)
                    const soccerFixtures = document.querySelectorAll('div.cpm-ParticipantFixtureDetailsSoccer');
                    console.log(`📋 Found ${soccerFixtures.length} soccer fixtures using direct selector`);
                    ...
                    for (const fixture of soccerFixtures) {
                        ...
                        const fixtureText = fixture.textContent.trim();
                        console.log(`🔍 Checking fixture: "${fixtureText}"`);

                        // Check 1: Skip if fixture contains eSoccer indicators
                        const fixtureTextLower = fixture.textContent.toLowerCase();

                        // Expanded eSoccer detection patterns
                        const eSoccerIndicators = [
                            'esoccer', 'e-soccer', 'virtual', 'simulated', 'esports',
                            'gaming', 'fantasy', 'fake', 'mock', 'cyber', 'digital',
                            'electronic', 'online', 'web', 'internet', 'computer',
                            'generated', 'artificial', 'synthetic', 'robot', 'ai',
                            'machine', 'automated', 'programmed', 'software'
                        ];

                        let isESoccer = false;
                        for (const indicator of eSoccerIndicators) {
                            if (fixtureTextLower.includes(indicator)) {
                                isESoccer = true;
                                console.log(`🚫 Skipping eSoccer fixture (indicator: ${indicator})`);
                                break;
                            }
                        }
                        ...
                        // Check 2: Skip if team names have eSoccer patterns (ENHANCED)
                        const teamElems = fixture.querySelectorAll('div.cpm-ParticipantFixtureDetailsSoccer_Team');
                        let hasESoccerPattern = false;
                        
                        // Known eSoccer gaming nicknames from the HTML samples
                        const eSoccerNicknames = [
                            'FAME', 'HAIL', 'DREAD', 'VENUS', 'DART', 'SPARTAN', 'CRUSADER', 
                            'ASSAULT', 'ZOOM', 'PHENOM', 'INSTINCT', 'WARRIOR', 'LEGEND', 
                            'BEAST', 'TITAN', 'VIPER', 'STORM', 'FURY', 'SHADOW', 'BLADE',
                            'DEMON', 'GHOST', 'KING', 'MASTER', 'SAVAGE', 'REBEL', 'HERO'
                        ];
                        
                        for (const teamElem of teamElems) {
                            const teamText = teamElem.textContent.trim();
                            console.log(`🔍 Checking team: "${teamText}"`);
                            
                            // Pattern 1: eSoccer teams have format: "TeamName (NICKNAME)"
                            if (teamText.includes('(') && teamText.includes(')')) {
                                const parenContent = teamText.match(/\\(([^)]+)\\)/);
                                if (parenContent && parenContent[1]) {
                                    const nickname = parenContent[1].toUpperCase().trim();
                                    
                                    // Skip youth teams and legitimate club abbreviations
                                    const legitimateAbbrevs = [
                                        'FC', 'SC', 'CF', 'AC', 'AFC', 'BFC', 'CFC', 'DFC', 'EFC', 'GFC', 
                                        'HFC', 'LFC', 'MFC', 'RFC', 'SFC', 'TFC', 'UFC', 'VFC', 'WFC',
                                        'YOUTH', 'U21', 'U19', 'U18', 'U17', 'U16', 'U15', 'II', 'III',
                                        'RESERVES', 'B', 'ACADEMY'
                                    ];
                                    
                                    // Check if it's a legitimate abbreviation
                                    let isLegitimate = false;
                                    for (const abbrev of legitimateAbbrevs) {
                                        if (nickname.includes(abbrev) || teamText.toLowerCase().includes('youth') || 
                                            teamText.toLowerCase().includes('reserve') || nickname.match(/^[IVXLCDM]+$/)) {
                                            isLegitimate = true;
                                            break;
                                        }
                                    }
                                    ...
                                    if (!isLegitimate) {
                                        // Check if nickname matches known eSoccer patterns
                                        if (eSoccerNicknames.includes(nickname) || 
                                            (nickname.length > 2 && nickname.length < 12 && 
                                             nickname.match(/^[A-Z]+$/) && !nickname.includes('FC'))) {
                                            console.log(`🚫 eSoccer pattern detected: "${nickname}"`);
                                            hasESoccerPattern = true;
                                            break;
                                        }
                                    }
                                }
                        ...
                        if (hasESoccerPattern) {
                            console.log('🚫 Skipping eSoccer fixture (nickname pattern)');
                            continue;
                        }

                        // Check 3: Skip if parent section is eSoccer (ENHANCED)
                        const parentSection = fixture.closest('.cpm-CouponPodModule, .ss-HomepageSpotlight');
                        if (parentSection) {
                            const sectionText = parentSection.textContent.toLowerCase();
                            const sectionTitle = parentSection.querySelector('.cpm-Header_Title, .ss-HomeSpotlightHeader_Title');
                            const titleText = sectionTitle ? sectionTitle.textContent.toLowerCase().trim() : '';

                            // Check section title for eSoccer indicators
                            const eSoccerTitles = ['esoccer', 'e-soccer', 'e soccer', 'virtual soccer', 'cyber soccer'];
                            let isESoccerSection = false;
                            for (const eSoccerTitle of eSoccerTitles) {
                                if (titleText.includes(eSoccerTitle)) {
                                    console.log(`🚫 Skipping eSoccer fixture (section title: "${titleText}")`);
                                    isESoccerSection = true;
                                    break;
                                }
                            }
                            ...
                            // Check section icon - CRITICAL: eSports icon indicates eSoccer
                            const headerImg = parentSection.querySelector('.cpm-Header_HeaderImage, .ss-HomeSpotlightHeader_HeaderImage');
                            if (headerImg) {
                                const imgStyle = headerImg.getAttribute('style') || '';
                                if (imgStyle.includes('zClass_ESports.svg')) {
                                    console.log(`🚫 Skipping eSoccer fixture (eSports icon detected)`);
                                    continue;
                                }
                            }
                            
                            // Additional check: If section contains multiple team names with gaming nicknames
                            const allTeamsInSection = parentSection.querySelectorAll('div.cpm-ParticipantFixtureDetailsSoccer_Team');
                            let eSoccerTeamCount = 0;
                            for (const teamElem of allTeamsInSection) {
                                const teamText = teamElem.textContent.trim();
                                if (teamText.includes('(') && teamText.includes(')')) {
                                    const parenContent = teamText.match(/\\(([^)]+)\\)/);
                                    if (parenContent && parenContent[1]) {
                            ...
                            // If more than 50% of teams have gaming nicknames, it's likely eSoccer
                            if (eSoccerTeamCount > allTeamsInSection.length * 0.5 && allTeamsInSection.length > 2) {
                                console.log(`🚫 Skipping eSoccer fixture (section has ${eSoccerTeamCount}/${allTeamsInSection.length} teams with gaming nicknames)`);
                                continue;
                            }
                        }
                    ...
                    // Method 2: Look for fixtures in soccer sections (backup with eSoccer filtering)
                    if (allFixtures.length === 0) {
                        // Find soccer sections first (exclude eSoccer sections)
                        let soccerSections = [];

                        // Look for soccer logo in style background-image (exclude eSports)
                        const headerImages = document.querySelectorAll('.cpm-Header_HeaderImage, .ss-HomeSpotlightHeader_HeaderImage');
                        for (const img of headerImages) {
                            const style = img.getAttribute('style') || '';
                            if ((style.includes('zClass_Soccer.svg') || style.includes('soccer')) &&
                                !style.includes('esoccer') && !style.includes('e-soccer') &&
                                !style.includes('zClass_ESports.svg')) {
                                const section = img.closest('.cpm-CouponPodModule, .ss-HomepageSpotlight');
                                if (section && !soccerSections.includes(section)) {
                                    // Additional check: make sure section doesn't contain esoccer text
                                    const sectionText = section.textContent.toLowerCase();
                                    if (!sectionText.includes('esoccer') && !sectionText.includes('e-soccer')) {
                                        soccerSections.push(section);
                                    }
                                }
                            }
                        }

                        // Extract fixtures from soccer sections
                        for (const soccerSection of soccerSections) {
                            const fixtureSelectors = [
                                'div.cpm-ParticipantFixtureDetailsSoccer',
                                'div[class*="ParticipantFixture"]',
                                'div.gl-MarketFixture',
                                'div.rcl-MarketFixtureDetailsLabelBase'
                            ];
                            ...
                            for (const selector of fixtureSelectors) {
                                const fixtures = Array.from(soccerSection.querySelectorAll(selector))
                                    .filter(fixture => !fixture.className.includes('Hidden') && fixture.offsetParent !== null);
                                allFixtures = allFixtures.concat(fixtures);
                            }
                        }
                    ...
                    console.log(`📋 Found ${uniqueFixtures.length} unique REAL soccer fixtures (eSoccer filtered out)`);

                    // Step 2: Extract match date with fallback for pregame
                    const dateHeaders = document.querySelectorAll('div.cpm-MarketFixtureDateHeader');
                    let matchDate = dateHeaders.length > 0 ? dateHeaders[0].textContent.trim() : null;

                    // Fallback to today's date for pregame if no date found
                    if (!matchDate || matchDate === null) {
                        const today = new Date();
                        const options = { weekday: 'short', month: 'short', day: 'numeric' };
                        matchDate = today.toLocaleDateString('en-US', options);
                    }

                    // Step 3: Extract odds columns from the page
                    const oddsColumns = {};
                    const marketOdds = document.querySelectorAll('div.cpm-MarketOdds');
                    ...
                    for (const section of marketOdds) {
                        ...
                        if (header) {
                            const headerText = header.textContent.trim();
                            if (headerText === '1' || headerText === 'X' || headerText === '2' || headerText === 'Money' || headerText === '1X2' || headerText === 'Match Winner') {
                                const columnOdds = [];
                                const oddsElems = section.querySelectorAll('span.cpm-ParticipantOdds_Odds');
                                for (const oddsElem of oddsElems) {
                                    const oddsText = oddsElem.textContent;
                    ...
                    // Step 4: Process ALL fixtures and extract team information
                    for (let i = 0; i < uniqueFixtures.length; i++) {
                        const fixture = uniqueFixtures[i];
                        ...
                        // Check for eSoccer nickname pattern again
                        ...
                        // Create unique match signature to avoid duplicates within soccer extraction
                        ...
                        console.log(`✅ REAL Soccer Match: ${team1} vs ${team2}`);

                        // Extract match time using soccer-specific selector
    ...
    async def handle_pregame_history(self, current_result: Dict[str, Any]):
        ...
        try:
            ...
            current_file = self.outputs_dir / "current_pregame_data.json"
    ...
    async def scrape_all_sports(self) -> Dict[str, Any]:
        ...
        output_file = self.outputs_dir / "current_pregame_data.json"
    ...
    async def navigate_to_bet365(self):
        """Navigate to bet365.ca for monitoring"""
        try:
