...
from pregame_new import EnhancedIntelligentScraper, Game, GameOdds
...
class RealTimeMonitor:
    ...
    def __init__(self, update_interval: float = 0.3):
        ...
        # File paths - use pregame files only (no separate live data file)
        self.output_dir = Path("outputs")
        self.output_dir.mkdir(parents=True, exist_ok=True)
        self.current_file = self.output_dir / "current_pregame_data.json"    # Use pregame file
        self.history_file = self.output_dir / "live_history.json"         # Live history
        self.pregame_history_file = self.output_dir / "pregame_history.json"  # Pregame history
        ...
        self.realtime_dir = Path("outputs/realtime")
        self.realtime_dir.mkdir(parents=True, exist_ok=True) 
        self.monitoring_log_file = self.realtime_dir / "monitoring_activity.json"
        self.cycle_stats_file = self.realtime_dir / "cycle_statistics.json"
    ...
    def setup_logging(self):
        ...
        log_dir = Path("outputs/realtime/logs")
    ...
    async def initialize(self):
        ...
        try:
            ...
            # Initialize scraper
            self.scraper = EnhancedIntelligentScraper(headless=True, load_wait=10000)
    ...
    async def start_monitoring(self):
        ...
        try:
            ...
            if not sports_to_monitor:
                ...
                sports_to_monitor = {'NBA', 'NHL', 'NFL', 'MLB', 'NCAAF', 'Tennis', 'UFC', 'CFL', 'PGA'}
    ...
    async def log_sport_activity(self, sport: str, games_found: int, duration: float, changes_detected: bool):
        ...
        try:
            ...
            if self.monitoring_log_file.exists():
                async with aiofiles.open(self.monitoring_log_file, 'r') as f:
                    data = json.loads(await f.read())
            ...
            async with aiofiles.open(self.monitoring_log_file, 'w') as f:
    ...
    async def write_data_files(self):
        """Update existing pregame JSON with realtime odds data"""
        try:
            # Read existing pregame data structure
            existing_data = {}
            if self.current_file.exists():
                try:
                    async with aiofiles.open(self.current_file, 'r') as f:
                    ...
                    self.logger.warning(f"Could not read existing pregame data: {e}")
            
            # Update existing data with realtime odds while preserving structure
            if existing_data and "sports_data" in existing_data:
                # Update extraction info timestamp
                if "extraction_info" in existing_data:
                    existing_data["extraction_info"]["timestamp"] = datetime.now().isoformat()
                
                # Update games with latest odds from realtime monitor
                for sport_name, sport_data in existing_data["sports_data"].items():
                    if "games" in sport_data:
                        for game_dict in sport_data["games"]:
                            # Find matching game in realtime data by game_id or team names
                            game_id = self._generate_game_id_from_dict(game_dict)
                            if game_id in self.current_games:
                                realtime_game = self.current_games[game_id]
                                # Update odds with realtime data
                                game_dict["odds"] = {
                                    "spread": realtime_game.odds.spread if realtime_game.odds else [],
                                    "total": realtime_game.odds.total if realtime_game.odds else [],
                                    "moneyline": realtime_game.odds.moneyline if realtime_game.odds else [],
                                    "runline": realtime_game.odds.runline if realtime_game.odds else []
                                }
                ...
                # Fallback: create simple format if no existing structure
                current_data = {
                    "timestamp": datetime.now().isoformat(),
                    "total_games": len(self.current_games),
                    "games": [self.game_to_dict(game) for game in self.current_games.values()]
                }
                async with aiofiles.open(self.current_file, 'w') as f:
            ...
            cycle_data = {
                ...
                "sport_update_counts": self.sport_update_counts,
                "total_active_games": len(self.current_games),
                "uptime_seconds": self.stats.uptime_seconds,
                "recent_cycles": self.cycle_history[-10:] if self.cycle_history else []
            }
            
            async with aiofiles.open(self.cycle_stats_file, 'w') as f:
                await f.write(json.dumps(cycle_data, indent=2))
