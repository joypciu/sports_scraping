...
class UltimateLiveScraper:
    ...
    def __init__(self, disable_broadcasting=False):
        ...
        self.current_data_file = os.path.join(self.script_dir, "bet365_live_current.json")
        self.history_data_file = os.path.join(self.script_dir, "bet365_live_history.json")
        self.standardized_data_file = os.path.join(self.script_dir, "bet365_live_standardized.json")
        self.selector_db_file = os.path.join(self.script_dir, "bet365_selectors_detailed.json")
        self.log_file = os.path.join(self.script_dir, f"bet365_scraper_{self.session_id}.log")
    ...
    def setup_logging(self):
        ...
        self.logger.info("Logging initialized - File: %s", self.log_file)

    def load_selector_database(self):
        """Load the detailed selector database"""
        try:
            if os.path.exists(self.selector_db_file):
                with open(self.selector_db_file, 'r', encoding='utf-8') as f:
                    data = json.load(f)
            else:
                data = {}
            ...
            if data:
                transformed_db = {
                    'last_updated': data.get('metadata', {}).get('completed_at', datetime.now().isoformat()),
                    'sports': {},
                    'global_selectors': {
                ...
                for url, sport_info in sports_data.items():
                    sport_code = sport_info.get('sport_code', 'Unknown')
                    selectors_working = sport_info.get('selectors_working', {})

                    transformed_selectors = {}
                    for selector_type, selectors in selectors_working.items():
                        if selector_type == 'teams':
                            transformed_selectors['team_names'] = selectors
                        elif selector_type == 'scores':
                            transformed_selectors['scores'] = selectors
                        elif selector_type == 'odds':
                            transformed_selectors['odds'] = selectors
                        elif selector_type == 'status':
                            transformed_selectors['status'] = selectors
                        else:
                            transformed_selectors[selector_type] = selectors

                    transformed_db['sports'][sport_code.lower()] = {
                        'last_updated': sport_info.get('analyzed_at', datetime.now().isoformat()),
                        'successful_selectors': transformed_selectors,
                        'elements_found': sport_info.get('elements_count', {}),
                        'usage_count': 0,
                        'url': url
                    }

                self.logger.info("Loaded detailed selector database with %d sports from %d URLs",
                               len(transformed_db['sports']), len(sports_data))
                return transformed_db
            else:
                return self._create_default_selector_database()
                
        except Exception as e:
            self.logger.error("Error loading detailed selector database: %s", e)
            return self._create_default_selector_database()
    ...
    def save_selector_database(self, db):
        """Save the selector database"""
        try:
            with open(self.selector_db_file, 'w', encoding='utf-8') as f:
                json.dump(db, f, indent=2, ensure_ascii=False)
            self.logger.debug("Selector database saved")
        except Exception as e:
            self.logger.error("Failed to save selector database: %s", e)
    ...
    async def extract_live_betting_data(self, page, sport_code='B1'):
        ...
        # Use comprehensive extraction script from comprehensive_extraction_script.py
        from comprehensive_extraction_script import get_comprehensive_extraction_script
    ...
    def save_live_results(self, results: Dict):
        ...
        statistics_filename = os.path.join(script_dir, "bet365_statistics.json")
